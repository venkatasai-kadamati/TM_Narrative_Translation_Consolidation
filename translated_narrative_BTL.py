...
output = os.path.join(fp, "output/CNB Report Narratives - BTL new.docx")
...
for x in natsorted(ruleIDs):
    ...
    for row in threshold_decisions.index:
        ...
        if temp_param in currencyF:
            temp_values_formatted = [f"${value:,.2f}" for value in temp_values]
        elif temp_param in numberF:
            temp_values_formatted = [f"{value:,}" for value in temp_values]
    ...
    for pop in popGroups:
        ...
        paragraph = doc.add_paragraph(narratives_Rule_Pop["Conclusion"].iloc[0])

        # Add placeholder for scoring
        # heading = doc.add_heading('Scoring Recommendation', level=2)
