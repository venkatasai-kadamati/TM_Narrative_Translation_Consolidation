...
report_name = "CNB - Production Report Narratives.docx"
...
# Data is loaded
data = pd.read_excel(fp + file_name, sheet_name=1)
    ...
    for pop in popGroups:
        ...
        for threshold in params:
            ...
            line_one = (
                "No alerts generated"
                if temp == 0
                else f"Production tuning analysis was performed on the {params_used} for the {data_Rule_Pop_Pop['Population Group'].values[0]} population group."
            )
            ...
            line_two = (
                ...
                if temp == 0
                    ...
                    if temp == 1
                    else f"{numbers[str(temp)]} rule breaks were generated"
                )
            ...
            line_three = (
                ...
                if temp == 0
                    ...
                    if temp == 1
                    else f"The Bank identified {numbers[str(temp)]} Data Quality rule breaks"
                )
            ...
            if threshold in currencyF:
                ...
                temp_formatted = numbers[str(temp)] if temp < 10 else f"{temp:,}"
            ...
            if threshold in currencyF:
                ...
                val_formatted = [
                    numbers[str(val)] if val < 10 else f"{val:,}"
                    for val in [
                        data_Rule_Pop_Parameter["Min Val"].values[0],
                        data_Rule_Pop_Parameter["Max Val"].values[0],
                    ]
            ...
            line_seven = (
                ...
                if data_Rule_Pop_Parameter["Num Alerts Extracted"].values[0] == 0
                    ...
                    f"Rule breaks were generated solely at a value of {val_formatted[1]} within the {data_Rule_Pop_Pop['Population Group'].values[0]} population segment."
                    if temp == 0
                    else f"Rule breaks were generated for values ranging between {val_formatted[0]} and {val_formatted[1]} within the {data_Rule_Pop_Pop['Population Group'].values[0]} population segment."
                )
            )
            ...
            if threshold in currencyF:
                ...
                temp_formatted = [
                    numbers[str(val)] if val < 10 else f"{val:,}" for val in temp
                ]
            elif threshold in percentF:
...
for x in ruleIDs:
    ...
    for _, row in threshold_decisions.iterrows():
        ...
        cells[2].text = format_threshold(
            row["Current Threshold"],
            row["Parameter"],
            currencyF,
            numberF,
            percentF,
        ...
        cells[3].text = format_threshold(
            row["Recommended Threshold"],
            row["Parameter"],
            currencyF,
            numberF,
            percentF,
...
def format_threshold(value, parameter, currencyF, numberF, percentF, decimalF):
    if parameter in currencyF:
        return f"${value:,.2f}"
    elif parameter in numberF:
        return f"{value:,}"
    elif parameter in percentF:
        return f"{value:.0%}"
    elif parameter in decimalF:
        return f"{value:,.2f}"
    else:
        return str(value)
